...
export function PaymentConfirmationScreen() {
  ...
  const { isWorking } = useTxFlowStatusModals({
    ...
    successMsg: `Please check your email/phone for a notification. Email support@chimoney.io with questions`,
  ...
  return (
    <ScreenContentFrame>
      <div css={style.content}>
        <h1 css={Font.h2Green}>Review Purchase</h1>
        ...
        {params.comment && (
          <Box direction="row" styles={style.inputRow} justify="between">
            <label css={style.labelCol}>Product</label>
            <label css={[style.valueLabel, style.valueCol]}>{tnxDetails['spendOn']}</label>
          </Box>
        )}

        {params.comment && (
          <Box direction="row" styles={style.inputRow} justify="between">
            <label css={style.labelCol}>Chimoney ID</label>
            <label css={[style.valueLabel, style.valueCol]}>{tnxDetails['issueID']}</label>
          </Box>
        )}
